...
$Win10Url = "https://raw.githubusercontent.com/BenMartinBaur/Windows-Virtual-Desktop/main/Win10MultiTemplate.json"
$Win10FileName = "Win10MultiTemplate.json"
#Test to see if the path exists.  Create it if not
if ((test-path .\02_Template) -eq $false) {
    new-item -ItemType Directory -name '02_Template'
} 
...
if ((test-path .\Template\$Win10FileName) -eq $true) {
    ...
    if ($confirmation -eq 'y' -or $confirmation -eq 'yes' -or $confirmation -eq 'Yes') {
        Invoke-WebRequest -Uri $Win10Url -OutFile ".\02_Template\$Win10FileName" -UseBasicParsing
    }
}
else {
    Invoke-WebRequest -Uri $Win10Url -OutFile ".\02_Template\$Win10FileName" -UseBasicParsing
...
$templateFilePath = ".\02_Template\$Win10FileName"
